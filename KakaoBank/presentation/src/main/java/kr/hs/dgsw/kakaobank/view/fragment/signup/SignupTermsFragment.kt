    ...
    override fun observerViewModel() {
        with(mViewModel) {
            termsIsCheck.observe(this@SignupTermsFragment, Observer {
                if (it) {
                    mBinding.signTNextBtn.setBackgroundColor(ContextCompat.getColor(requireContext(),
                        R.color.kakao))
                    mBinding.signTNextBtn.setTextColor(ContextCompat.getColor(requireContext(),
                        R.color.text_mainColor))
                    mBinding.signTNextBtn.setOnClickListener {
                        this@SignupTermsFragment.findNavController()
                            .navigate(R.id.action_signup_terms_to_signup_input)
                    }
                } else {
                    mBinding.signTNextBtn.setBackgroundColor(ContextCompat.getColor(requireContext(),
                        R.color.disabled))
            ...
            secondPolicy.observe(this@SignupTermsFragment, Observer {
                val intent =
                    Intent(Intent.ACTION_VIEW, Uri.parse("https://m.kakaobank.com/EBankingPolicy"))
                startActivity(intent)
            })
        }
    }
